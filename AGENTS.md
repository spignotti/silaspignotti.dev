...
`standard` — presentable portfolio work with repeatable validation; no release ceremony needed.

Git policy: `direct-main` — personal/local repo (commit on default branch, no worktree by default); feature branch only for larger non-trivial changes or explicit request. Worktrees only for parallel write work or explicit request.
...
- profile gates: `pnpm run build` before every commit; `pnpm run check` for structural changes, content schema changes, and before larger pushes
